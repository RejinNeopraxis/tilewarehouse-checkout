...
> {
    ...
    componentDidMount(): void {
        ...
        loadOrder(orderId)
            .then(({ data }) => {
                ...
                // In case the load script re-runs for whatever reason, don't add the
                // script again
                if (order && !document.getElementById('dotdigital-roi')) {
                    const script = document.createElement('script');
                    script.id = 'dotdigital-roi';
                    script.type = 'text/javascript';
                    const content = order.lineItems.physicalItems
                        .map((item) => `_dmTrack("product", "${item.name}");`)
                        .concat(
                            `_dmTrack("CheckOutAmount", "${order.orderAmount}");`
                        )
                        .join('\n');
    ...
    render(): ReactNode {
        ...
        return (
            <div
                className={classNames('layout optimizedCheckout-contentPrimary layout-order-confirmation', {
